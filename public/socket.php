    ...
    foreach ($read_sockets as $socket) {
        ...
        switch (count($tk103_data)) {
            ...
            case 19: 
                ...
                {
                    verifi_range($imei,$latitude,$longitude);
                }
		...
		if ($alarm == "help me") {
		    ...
		    insert_notificacion($imei,"Ocurrio una alerta de ayuda","help me");
                }
        if($alarm=="acc off")
        {
            insert_notificacion($imei,"Se desconecto la bateria","acc off");
        }
        if($alarm=="speed")
        {
            insert_notificacion($imei,"Aumento de la velocidad","speed");
        }
        break;
        }
        if (!$data) {
...
function verifi_range($imei,$latitude,$longitude)
{
    $polygon=array();
	...
	$query = "select * from rango";
        foreach($conn->query($query ) as $fila) {
          ...
          {
            insert_notificacion($imei,"fuera de rango","rango");
          }
...
function insert_notificacion($imei,$mensaje,$tipoalerta)
{   
    $alerta_permitida=0;
    $servername = "localhost";
    $username = "root";
